...
export default async (plugin: Plugin) => {
  ...
  async function request(url: string, data: string, options: any) {
  ...
  plugin
    ...
    .defun("protocol", (url) =>
      plugin.from_string(plugin.unbox<URL>(url).protocol)
    )
    .defun("username", (url) =>
      plugin.from_string(plugin.unbox<URL>(url).username)
    )
    .defun("password", (url) =>
      plugin.from_string(plugin.unbox<URL>(url).password)
    )
    .defun("host", (url) => plugin.from_string(plugin.unbox<URL>(url).host))
    .defun("hostname", (url) =>
      plugin.from_string(plugin.unbox<URL>(url).hostname)
    )
    .defun("port", (url) =>
      plugin.from_integer(BigInt(plugin.unbox<URL>(url).port))
    )
    .defun("pathname", (url) =>
      plugin.from_string(plugin.unbox<URL>(url).pathname)
    )
    .defun("query", (url0) => {
      const url = plugin.unbox<URL>(url0);
      return get_search(url);
    })
    .defun("hash", (url) => plugin.from_string(plugin.unbox<URL>(url).hash))
    .defun("to-text", (url) =>
      plugin.from_string(plugin.unbox<URL>(url).toString())
    )
