...
export default async (plugin: Plugin) => {
  plugin
    .define_ffi("path")
    .defun("basename", (path) =>
      plugin.from_string(Path.basename(plugin.get_string(path)))
    )
    .defun("basename-ext", (path, ext) =>
      plugin.from_string(
        Path.basename(plugin.get_string(path), plugin.get_string(ext))
      )
    )
    .defun("delimiter", () => plugin.from_string(Path.delimiter))
    .defun("dirname", (path) =>
      plugin.from_string(Path.dirname(plugin.get_string(path)))
    )
    .defun("extname", (path) =>
      plugin.from_string(Path.extname(plugin.get_string(path)))
    )
    .defun("is-absolute", (path) =>
      plugin.from_bool(Path.isAbsolute(plugin.get_string(path)))
    )
    .defun("join", (xs) =>
      plugin.from_string(
        Path.join(...plugin.get_array(xs).map((x) => plugin.get_string(x)))
      )
    )
    .defun("normalise", (path) =>
      plugin.from_string(Path.normalize(plugin.get_string(path)))
    )
    .defun("relative", (from, to) =>
      plugin.from_string(
        Path.relative(plugin.get_string(from), plugin.get_string(to))
      )
    )
    .defun("resolve", (xs) =>
      plugin.from_string(
  ...
  plugin
    ...
    .defun("exists", (path) =>
      plugin.from_bool(FS.existsSync(plugin.get_string(path)))
    )
    .defun("mkdir", (path) => {
    ...
    .defun("readdir", (path) => {
      return plugin.from_array(
        FS.readdirSync(plugin.get_string(path)).map((x) =>
          plugin.from_string(x)
        )
      );
    })
    .defun("read-file-text", (path) => {
      return plugin.from_string(
        FS.readFileSync(plugin.get_string(path), "utf8")
      );
    })
    ...
    .defun("is-directory", (path) => {
      return plugin.from_bool(
        FS.statSync(plugin.get_string(path)).isDirectory()
      );
    });
};
