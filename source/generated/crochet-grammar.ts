...
export class $$Declaration$_ForeignType extends Declaration {
  ...
  constructor(
    ...
    $assert_type<Meta>(pos, "Meta", Meta);
    $assert_type<Metadata>(cmeta, "Metadata", Metadata);
    $assert_type<Name>(name, "Name", Name);
    $assert_type<Namespace>(target, "Namespace", Namespace);
  }
...
export class $$Declaration$_Action extends Declaration {
  ...
  constructor(
    readonly pos: Meta,
    readonly cmeta: Metadata,
    readonly self: TypeApp,
    readonly name: Name,
    readonly title: Expression | null,
    readonly pred: Predicate,
    readonly rank: Rank,
    readonly body: Statement[],
    readonly commands: TypeInit[]
  ) {
    super();
    Object.defineProperty(this, "tag", { value: "Action" });
    $assert_type<Meta>(pos, "Meta", Meta);
    $assert_type<Metadata>(cmeta, "Metadata", Metadata);
    $assert_type<TypeApp>(self, "TypeApp", TypeApp);
    $assert_type<Name>(name, "Name", Name);
    $assert_type<Expression | null>(
      title,
      "(Expression | null)",
      $is_maybe(Expression)
    );
    ...
    $assert_type<Statement[]>(body, "Statement[]", $is_array(Statement));
    $assert_type<TypeInit[]>(commands, "TypeInit[]", $is_array(TypeInit));
  }
...
export class $$Expression$_ForeignInvoke extends Expression {
  ...
  constructor(
    ...
    $assert_type<Meta>(pos, "Meta", Meta);
    $assert_type<Namespace>(name, "Namespace", Namespace);
    $assert_type<Expression[]>(args, "Expression[]", $is_array(Expression));
  }
...
function get_semantics() {
  ...
  semantics = get_grammar().createSemantics();
...
export function parse(source: string, rule: string): Result<Program> {
  const result = get_grammar().match(source, rule);
  if (result.failed()) {
...
export const toAstVisitor = {
  ...
  castExpression_alt1(
    ...
    return new Expression.Invoke(
      $meta(this),
      new Signature.Binary($meta(this), op, s, t)
    );
  },
  ...
  asParameter_alt1(this: Ohm.Node, t$0: Ohm.Node): any {
    ...
    return new Parameter.TypedOnly(
      $meta(this),
      new TypeApp.Static($meta(this), t)
    );
  },
...
export function toAst(result: Ohm.MatchResult) {
  return get_semantics()(result).toAST();
}
